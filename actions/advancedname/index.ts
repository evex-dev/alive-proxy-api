...
type ENDPOINT_TYPE = `https://advanced.name/freeproxy?page=${number}`;

const GenerateEndpoint = () =>
  `https://advanced.name/freeproxy?page=${
...
export async function AdvancednameAction(): Promise<ACTIONS_RESULT> {
  ...
  const rowElements = document.querySelectorAll(
    "#table_proxies > tbody > tr:nth-child(n)",
  );
  ...
  for (let i = 0; i < cachedElementsLength; i++) {
    ...
    const ip = atob(
      String(
        ...
        rowElement.querySelector("td:nth-child(2)").getAttribute("data-ip"),
      ),
    );
    const port = Number(
      atob(
        String(
          ...
          rowElement.querySelector("td:nth-child(3)").getAttribute("data-port"),
        ),
      ),
    );
    const protocolElements: Node[] = [
      // @ts-expect-error NOT TYPED WELL

      ...rowElement.querySelectorAll("td:nth-child(4) > a"),
    ];
    ...
    const country = String(
      ...
      (rowElement.querySelector("td:nth-child(5) > a") ?? {
        textContent: "XX",
      }).textContent,
    );
